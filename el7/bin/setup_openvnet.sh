...
systemctl start vnet-vnmgr
systemctl start vnet-webapi
...
if [ "${network_addr}" == "" ]; then
  network_addr="10.1.12.0"
fi

host_if='eth2'
inetary=($(ip addr show dev br0 | grep 'inet'))

host_addr=$(echo ${inetary[1]} | awk -F '[/]' '{print $1}')
host_mask=$(echo ${inetary[1]} | awk -F '[/]' '{print $2}')

infoary=($(ip addr show dev br0 | grep 'link/ether'))
host_mac=${infoary[1]}
